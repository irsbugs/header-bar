...
class Main_Window(Gtk.Window):
    def __init__(self):
        ...
        self.header_bar.set_title("Double Precision Demo")
        self.header_bar.set_subtitle("2021-05-10")        
        
        # Add widgets using traditional method to the Gtk.Window
        self.grid = Gtk.Grid()
        self.grid.set_border_width(10)
        self.label = Gtk.Label(label=
                "Viewing Double Precision / 64 Bit / IEEE754 Floating Point Data")
        ...
        # setup the 64 bit display with buttons
    ...
    def setup_sign_adjustment(self):
        ...
        checkbutton_sign = Gtk.CheckButton(label="Unchecked positive / Checked negative")
        checkbutton_sign.connect("clicked", self.cb_sign_adjust)
        
        grid_adjust.attach(checkbutton_sign, 0,0,1,1)
        
        
    def setup_exponent_adjustment(self):
        """Setup the exponent quick adjustments in a button box."""
        frame = Gtk.Frame(label="Exponent")
        frame.set_label_align(0.1,0.5)
        frame.get_style_context().add_class("frame_main")        
    ...
    def ieee754_breakdown(self, index = 0):
        ...
        for i in range(0, 52):
            s += self.main_button_bit_list[index][i].get_label()
        s = s[::-1]
        print(s)  # left to right 0 to 2**-52               
        fraction = 0 
        ...
        # Calculation...
        # (-1)**1 sign bit *   (1+fraction)  * 2 exponent - bias
        #>>> (-1)**0
        #1
        #>>> (-1)**1
        #-1 
        ...
        try:
            if fraction == 0 and exponent == -1023 and sign == 0:
                ...
                print(self.main_frame_list[index].get_label())
                self.main_frame_list[index].set_label(self.main_frame_list[index].get_label() + 
        ...
        Traceback (most recent call last):
          File "double_precision.py", line 389, in cb_button_bit
            self.ieee754_breakdown()
          File "double_precision.py", line 186, in ieee754_breakdown
            decimal_value = (-1)**sign * (1 + fraction) * (2**exponent)
...
def add_provider(widget):
    ...
    /* Provide the background colouring for the components of the insttruction */
    #col_0 {
        background: Aquamarine;
        /*background: rgb(255,160,160)*/
        }
